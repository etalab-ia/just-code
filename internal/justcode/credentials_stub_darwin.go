...
// (darwin compiles only the Keychain adapter). They satisfy the
// CredentialStore interface so DefaultCredentialStore's references
// resolve, but every operation reports the store as unavailable: the
// platform cannot use it, and the CLI must not pretend otherwise.
