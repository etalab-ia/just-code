...
Le provisionnement est volontairement minimal : les paquets de base, Node.js et OpenCode, plus un `PATH` posé dans `/etc/profile.d/just-code.sh` pour que le binaire soit visible quel que soit le shell de connexion. just-code ne construit ce template que sous son nom par défaut : tout autre `AGENT_VM_TEMPLATE` désigne un template que vous maintenez, et il n'est ni construit ni remplacé. C'est le point de personnalisation — une équipe peut y préinstaller Docker, Chromium ou d'autres agents et le désigner via `AGENT_VM_TEMPLATE` ; le clonage reste identique.

La construction est refaite uniquement si le template est absent. Pour la forcer (après un changement de ressources, par exemple), supprimez d'abord l'instance : `limactl delete agent-vm-base --force`.
